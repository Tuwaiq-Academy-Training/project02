...
//***********************************UPDATE MESSAGES****************************************
app.put('/update/:groupname/:name', (req,res)=>{
    if(req.params.groupname=='homeworks'){
    let user = homeworks.find((elm)=> elm.name == req.params.name);
    user.msg = req.body.msg ;
    res.json(homeworks);

   }else if(req.params.groupname=='random-chating'){
    let user = randomchating.find((elm)=> elm.name == req.params.name);
    user.msg = req.body.msg ;
   res.json(randomchating);

   }else if(req.params.groupname=='announcements'){
    let user = ann.find((elm)=> elm.name == req.params.name);
    user.msg = req.body.msg ;
   res.json(ann);

  }else if(req.params.groupname=='q-and-a'){
    let user = q.find((elm)=> elm.name == req.params.name);
    user.msg = req.body.msg ;
  res.json(q);}
  });

//************************************delete message*************************************** 
app.delete('/delete/:groupname/:name', (req,res)=>{
    if(req.params.groupname=='homeworks'){
   homeworks = homeworks.filter((elm)=> elm.name !== req.params.name);
    res.json(homeworks);

   }else if(req.params.groupname=='random-chating'){
    randomchating = randomchating.filter((elm)=> elm.name !== req.params.name);
   res.json(randomchating);
  

   }else if(req.params.groupname=='announcements'){
    ann = ann.filter((elm)=> elm.name !== req.params.name);
    res.json(ann);

  }else if(req.params.groupname=='q-and-a'){
    q = q.filter((elm)=> elm.name !== req.params.name);
    res.json(q);}
  });
